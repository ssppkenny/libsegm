...
typedef boost::adjacency_list < boost::vecS, boost::vecS, boost::undirectedS, int > RectsGraph;
...
std::vector<std::pair<interval<int>,interval<int>>> all_pairs(std::vector<interval<int>> intervals) {
    ...
    for (int i = 0; i < size; i++) {
        for (int j = i+1; j < size; j++) {
            return_value.push_back(std::make_pair(intervals.at(i), intervals.at(j)));
        }
    }
    return return_value;
}

vector<cv::Rect> join_rects(vector<cv::Rect> rects) {
    ...
    for (const auto& value: rects) {
        ...
        numbered_rects.at(a) = i;
        i++;
        std::pair<int,int> map_key = std::make_pair(value.y, value.y + value.height);
        if (rects_dict.find(map_key) == rects_dict.end()) {
           std::vector<cv::Rect> v;
           v.push_back(value);
           rects_dict.at(map_key) = v;
        } else {
           auto& v = rects_dict.at(map_key);
           v.push_back(value);
        }
        sweep_event le;
        le.x = value.x;
    ...
    for (const auto& event: events) {
        ...
        if (event.type == Left) {
            ...
            tree.overlap_find_all({rect.y, rect.y + rect.height}, [&overlapping_intervals](auto iter) {
             overlapping_intervals.push_back(make_safe_interval(iter->low(), iter->high()));
             return true;
        });
        overlapping_intervals.push_back(make_safe_interval(rect.y, rect.y + rect.height));
        std::vector<std::pair<interval<int>,interval<int>>> pairs = all_pairs(overlapping_intervals);
        for (const auto& p : pairs) {
            std::vector<cv::Rect> r1filtered;
            std::vector<cv::Rect> r2filtered;
            std::vector<cv::Rect> r1s = rects_dict[std::make_pair(p.first.low(), p.first.high())];
            std::copy_if (r1s.begin(), r1s.end(), std::back_inserter(r1filtered), [&event](cv::Rect r){return event.x <= r.x + r.width && event.x >= r.x;} );
            cv::Rect r1 = r1filtered.at(0);

            std::vector<cv::Rect> r2s = rects_dict[std::make_pair(p.second.low(), p.second.high())];
            std::copy_if (r2s.begin(), r2s.end(), std::back_inserter(r2filtered), [&event](cv::Rect r){return event.x <= r.x + r.width && event.x >= r.x;} );
            cv::Rect r2 = r2filtered.at(0);

            int ind1 = numbered_rects.at(std::array<int,4> {r1.x, r1.y, r1.width, r1.height});
            int ind2 = numbered_rects.at(std::array<int,4> {r2.x, r2.y, r2.width, r2.height});
